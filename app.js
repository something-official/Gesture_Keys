...
let typedText = '';
function keyRect(width, height, rowIndex, keyIndex) {
  const keyWidth = Math.min(48, width / 12);
  const rowWidth = rows[rowIndex].length * (keyWidth + 5);
  return { x: (width - rowWidth) / 2 + rowIndex * keyWidth * 0.35 + keyIndex * (keyWidth + 5), y: height * (0.25 + rowIndex * 0.2), width: keyWidth, height: keyWidth };
}
function keyAt(width, height, x, y) {
  for (let rowIndex = 0; rowIndex < rows.length; rowIndex += 1) {
    for (let keyIndex = 0; keyIndex < rows[rowIndex].length; keyIndex += 1) {
      const rect = keyRect(width, height, rowIndex, keyIndex);
      if (x >= rect.x && x <= rect.x + rect.width && y >= rect.y && y <= rect.y + rect.height) return rows[rowIndex][keyIndex];
    }
  }
  return null;
}
...
function resetMode() {
  typedText = '';
  if (typedOutput) typedOutput.textContent = 'Typed output: —';
}
function drawScene(width, height, signal) {
  const cursorX = signal.centerX * width;
  const cursorY = signal.centerY * height;
  rows.forEach((row, rowIndex) => [...row].forEach((letter, keyIndex) => {
    const rect = keyRect(width, height, rowIndex, keyIndex);
    const hover = cursorX >= rect.x && cursorX <= rect.x + rect.width && cursorY >= rect.y && cursorY <= rect.y + rect.height;
    context.fillStyle = hover ? '#fef08a' : 'rgba(103, 232, 249, 0.18)';
    context.strokeStyle = hover ? '#fff' : '#67e8f9';
    context.lineWidth = 1.5;
    context.beginPath();
    context.roundRect(rect.x, rect.y, rect.width, rect.height, 6);
    context.fill();
    context.stroke();
    context.fillStyle = hover ? '#07111f' : '#e6fbff';
    context.font = '800 16px system-ui';
    context.textAlign = 'center';
    context.fillText(letter, rect.x + rect.width / 2, rect.y + rect.height * 0.62);
  }));
  context.textAlign = 'left';
  context.fillStyle = '#f0abfc';
  context.font = '800 15px system-ui';
  context.fillText('POINTER CLICK → APPEND', 18, height - 22);
  context.strokeStyle = '#fef08a';
  context.beginPath();
  context.arc(cursorX, cursorY, 12 + signal.brightness * 8, 0, Math.PI * 2);
  context.stroke();
}
canvas.addEventListener('pointerdown', (event) => {
  updatePointer(event);
  const letter = keyAt(canvas.clientWidth, canvas.clientHeight, pointer.x * canvas.clientWidth, pointer.y * canvas.clientHeight);
  if (letter) {
    typedText = `${typedText}${letter}`.slice(-32);
    if (typedOutput) typedOutput.textContent = `Typed output: ${typedText}`;
  }
});
